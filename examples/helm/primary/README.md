...
```console
$ helm install primary --name primary \
  --set Image.tag=centos7-9.6.10-2.2.0
```

The above command changes the image tag of the container from the default of `centos7-10.5-2.2.0` to `centos7-9.6.10-2.2.0`.
...
| `.image.tag` | The image tag to be used.    | `centos7-10.5-2.2.0`                                                    |
