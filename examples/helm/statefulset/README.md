...
```console
$ helm install statefulset --name statefulset \
  --set Image.tag=centos7-9.6.9-2.1.0
```

The above command changes the image tag of the container from the default of `centos7-10.4-2.1.0` to `centos7-9.6.9-2.1.0`.
...
| `.image.tag` | The image tag to be used.    | `centos7-10.4-2.1.0`                                                    |
