...
```console
$ helm install custom-config --name custom-config \
  --set Image.tag=centos8-13.4-5.0.3-0
```

The above command changes the image tag of the container to of `centos8-13.4-5.0.3-0`.
...
| `.image.tag` | The image tag to be used.    | `centos8-13.4-5.0.3-0`                                                    |
