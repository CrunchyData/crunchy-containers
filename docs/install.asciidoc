...
If you already have an NFS share available, you may specify this by setting the `NFS_SHARE_PATH` (and `NFS_SHARE_SERVER` if needed) environment variable(s), and configure permissions as described in this guide. Currently, these environment variables are only supported in the Kubernetes examples.
