...
This starts a pg_audit database container on port 12005 on localhost.  You can then
run the test script as follows:
....
./test-pgaudit.sh
....
...
The script will create an Audit pod on the Kubernetes instance and 
then executes a SQL file which contains pgaudit configuration
statements as well as executes some basic SQL commands.  These
SQL commands will cause pgaudit to create audit log messages in
the pg_log log file created by the database container.
...
The script will create an Audit pod on the OpenShift instance and 
then executes a SQL file which contains pgaudit configuration
statements as well as executes some basic SQL commands.  These
SQL commands will cause pgaudit to create audit log messages in
the pg_log log file created by the database container.
...
such as *centos7-9.5.11-1.8.0* prior to running this upgrade.
...
environment variable is using a PG 9.6 image such as *centos7-9.6.7-1.8.0*.
