 ...
 * each example will create resources such as Secrets, ConfigMaps, Services, PersistentVolumeClaims, all which following a naming convention of *<example name>-<optional description suffix>*, for example an example called *primary* might have a PersistentVolumeClaim called *primary-pgconf* to describe the purpose of that particular PVC
 * the example folder names gives a clue as to the Container Suite feature it demonstrates, for example the *examples/kube/pgaudit* example demonstrates how to enable the *pgaudit* capability of the *crunchy-postgres* container

== Usage

NOTE: The examples located in the `kube` directory work on both Kube and OpenShift. Set the CCP_CLI environment variable
...
==== Kubernetes and OpenShift Custom Configuration

This example shows how you can use your own customized version of *setup.sql*
when creating a PostgreSQL database container in Kubernetes and OpenShift.
...
....
cd $CCPROOT/examples/kube/customer-config-ssl
./run.sh
....
...
For production Kubernetes and OpenShift installations, you'll likely want DNS
...
$CCP_STORAGE_PATH/primary-pvc-backups in order to locate the backup to use. Edit the primary-restore.json
...
....
${CCP_CLI} describe pod pgadmin4 | grep Node:
....
...
....
${CCP_CLI} logs badger-example -c badger
....
...
link:https://github.com/crunchydata/crunchy-containers/blob/master/examples/kube/metrics/metrics-pod.json[Kubernetes and OpenShift]
...
....
${CCP_CLI} logs -c grafana crunchy-metrics
${CCP_CLI} logs -c prometheus crunchy-metrics
${CCP_CLI} logs -c collect crunchy-pgsql
${CCP_CLI} logs -c postgres crunchy-pgsql
....
...
environment variables passed into the job; these are defined with further detail
...
On Docker, his example is required to run in conjunction with another container, by default the
*primary-replica* example.

On Kubernetes and OpenShift, the example will deploy a *primary-replica*.
...
....
psql -h primary-pitr-restore -U postgres postgres -c 'table pitrtest'
psql -h primary-pitr-restore -U postgres postgres -c 'create table foo (id int)'
psql -h primary-pitr-restore -U postgres postgres -c 'select pg_wal_replay_resume()'
psql -h primary-pitr-restore -U postgres postgres -c 'create table foo (id int)'
....
