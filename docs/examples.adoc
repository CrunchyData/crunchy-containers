...
a ate PostgreSQL backup, it is assumed the user wants to perform a
database restore.
...
You can view the extensions that postgres-gis has enabled by running:
....
...
To exercise some of the basic PostGIS functionality for validation (in this case defining 2D geometry point for everyone's favorite place - Walt Disney World!), run the command:

....
postgres=# select ST_MakePoint(28.385200,-81.563900);
....
