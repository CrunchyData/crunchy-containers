...
On Docker, this example is required to run in conjunction with another container, by default the
*primary-replica* example.

On Kubernetes and OpenShift, the example will deploy a *primary-replica* equivalent set of pods and related services.
...
....
${CCP_CLI} scale statefulset --replica=3
....
...
The example causes Kubernetes to create the required persistent volume claims automatically:
....
${CCP_CLI} get pvc
NAME             STATUS    VOLUME                                     CAPACITY   ACCESSMODES   STORAGECLASS   AGE
pgdata-statefulset-0   Bound     pvc-06334f6f-371b-11e7-9bda-42010a8000e9   1Gi        RWX           slow           5m
pgdata-statefulset-1   Bound     pvc-063795b3-371b-11e7-9bda-42010a8000e9   1Gi        RWX           slow           5m
....
...
psql -h restore-pitr -U postgres postgres -c 'select pg_wal_replay_resume()'
psql -h restore-pitr -U postgres postgres -c 'create table foo (id int)'
....
...
`pg_xlog` instead.
....
psql -h restore-pitr -U postgres postgres -c 'select pg_xlog_replay_resume()'
....
