...
Postgres replicas are read-only whereas a master is both read and write
capable.
 ...
 * PG_SLAVE_SERVICE_NAME - database host to connect to for the replica node
 ...
 * PG_SLAVE_SERVICE - the replica service name
 ...
 * start watching the new master in case that falls over next
 * look for replicas that have the metadata label value of *slavetype=trigger* to prefer
