...
PostgreSQL metrics are collected by the crunchy-collect container. This container
...
pod that holds the crunchy-postgres container.
...
both master and replica, so that applications can only have to work
with a single database connection.

PostgreSQL replicas are read-only whereas a master is both read and write
capable.
 ...
 * create the trigger file on the replica that will become the new master
 * change the labels on the replica to be those of the master
 * will start watching the new master in case that falls over next
 * will look for replicas that have the metadata label value of *slavetype=trigger* to prefer
...
The crunchy-vacuum image is executed, passed in the Postgres connection parameters to the single-master
...
The crunchy-pgbouncer container executes the PostgreSQL pgbouncer
 ...
 * PG_SLAVE_SERVICE - the name of the replica database container, this is
