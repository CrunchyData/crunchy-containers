...
under /tmp/backups/master directory.  You will see a value like
...
You should not see the test table since we referenced the restore
label created prior to the test table being created.

Next, run the following script to restore the database
to the *afterchanges* restore point:
....
./run-restore-pitr-after.sh
....
