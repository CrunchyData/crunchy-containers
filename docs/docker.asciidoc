...
This example assumes you have run Example 1, and that the basic
container is running.

For running the master-slave configuration , you can run the following script:
....
cd $CCPROOT/examples/docker/master-replica/
./run.sh
....
...
....
cd $CCPROOT/examples/docker/dns
./run.sh
....
...
An example is shown in the $CCPROOT/examples/docker/run-vacuum.sh script
