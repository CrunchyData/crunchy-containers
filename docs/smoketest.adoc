...
If you wanted a more automated (or at least faster) way of setting these up, two shell scripts have been provided in the /docs directory, link:setup-env.sh
and link:setup-env-2.sh. Ensure setup-env.sh has the proper version numbers for what you are testing, and run that script first; then after following the
