...
such as *centos7-9.5-1.4.0* prior to running this upgrade.
...
environment variable is using a 9.6 image such as *centos7-9.6-1.4.0*.
...
Here is a link that describes how to set up persistent disk:
https://cloud.google.com/container-engine/docs/tutorials/persistent-disk/

You can however create GCE disk automatically by creating PV's using:
....
examples/pv/create-pv.sh gce
examples/pv/create-pvc.sh
....

Prior to running the above script, edit the examples/envvars.sh script to specify your
GCE environment settings used for creating the GCE disks.
