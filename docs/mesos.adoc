...
=== mesos Example 1 - master-replica
...
Running the example:
....
examples/mesos/master-replica/run.sh
...
psql -h pgreplica.marathon.mesos -U postgres postgres
....

This example currently uses a NFS mounted volume path to persist
both the master and replica data files.
